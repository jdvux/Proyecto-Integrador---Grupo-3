window.addEventListener('load', () => {
    ...
    form.addEventListener('submit', e => {
        ...
        if(nameField.value.trim() === '') {
            validForm = false;
            errors[0].innerHTML = 'El campo debe estar completo';
        }else if (nameField.value.length < 5) {
            validForm = false;
            errors[0].innerHTML = 'El campo nombre debe tener como mínimo 5 caracteres';
        }else {
            errors[0].innerHTML = '';
        }
        ...
        if (descriptionField.value.length < 20) {
            validForm = false;
            errors[1].innerHTML ='El campo descripción debe tener como mínimo 20 caracteres';
        }else {
            errors[1].innerHTML = '';
        }
        ...
        if (imageField.value == "" || !imageField.value.length) {
            validForm = false;
            errors[2].innerHTML = 'Debes subir al menos una imagen';
        }else {
            errors[2].innerHTML = '';
        }
